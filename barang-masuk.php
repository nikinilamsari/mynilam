        ...
        <div class="card" id="tampil">
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-lg-6">
                        <button class="btn bg bg-purple float-right" style="margin-right:5px" onclick="show('uprint2',null)"><i class="fas fa-print"></i> Cetak Detail</button>
                ...
                <table id="example2" class="table table-bordered table-striped">
                    ...
                    <tbody>
                        <?php
                        $no = 0;
                        $sql = "SELECT * FROM tb_barangmasuk a INNER JOIN tb_supplier b ON a.id_supplier=b.id_supplier";
                        foreach (cari_array($conn, $sql) as $key) :
                            $no++;
                        ?>
                            <tr>
                                <td align="center"><?= $no; ?></td>
                                <td align="center"> <button class="btn btn-primary btn-sm" onclick="show('detail',<?= $key['id_barangmasuk'] ?>)"><i class="fas fa-eye"></i></button> <button class="btn btn-danger btn-sm" onclick="show('hapus',<?= $key['id_barangmasuk'] ?>)"><i class="fas fa-trash-alt"></i></button></td>
                                <td><?= $key['kode_barangmasuk'] ?></td>
                                <td><?= $key['tgl_barangmasuk'] ?></td>
                                <td><?= $key['terimadari'] ?></td>
                    ...
                    <div class="bs-stepper-header" role="tablist">
                        ...
                        <div class="step" data-target="#logins-part">
                            <button type="button" class="step-trigger" role="tab" aria-controls="logins-part" id="logins-part-trigger">
                                ...
                                <span class="bs-stepper-label">Masukkan Data Masuk</span>
                            </button>
                        </div>
                        <div class="line"></div>
                                ...
                                <div class="row">
                                    <div class="col-lg-6">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="kdbrgmsk" id="kdbrgmsk" class="form-control" placeholder="Masukkan Kode Barang Masuk">
                                            <span class="help-block" id="help_kdbrgmsk"></span>
                                        </div>
                                        <div class="form-group">
                                    ...
                                    <div class="col-lg-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="trmbrg" id="trmbrg" class="form-control" placeholder="Masukkan  Terima Dari">
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="callout callout-success bg bg-green col-lg-5">
                        ...
                        <br>
                        <span><b>Supplier</b> : <span id="spp"></span></span>
                    </div>
                    <!-- <div class="col-lg-7">
...
<script>
    ...
    function show(act, id) {
        if (act == 'tambah') {
            ...
            var dt_get = getJSON('<?= base_url() ?>load/barang-masuk/getbarangmasuk.php', {
                id: id
            });
            // console.log(dt_get);
            // return;
            ...
            $('#stepper').hide()
            $('#form-data').hide();
            $('#id').val(dt_get.data.id_barangmasuk)
            $('#tambahForm .card-body').prepend('<p id="hapus-notif">Apakah Anda yakin ingin menghapus : <strong>' + dt_get.data.kode_barangmasuk + '</strong></p>');
            $('.y').show();

            // alert('Button Hapus ' + id)

        } else if (act == 'detail') {
            var dt_get = getJSON('<?= base_url() ?>load/barang-masuk/detail.php', {
                id: id
            });
            ...
            $.each(dt_get.data, function(index, isi) {
                ...
                $('#modal #tgg').html(isi.tgl_barangmasuk)
                $('#modal #spp').html(isi.nama_supplier)

                detail += "<tr>"
                detail += "<td>" + (index + 1) + "</td>"
                detail += "<td>" + isi.kode_barang + "</td>"
                detail += "<td>" + isi.nama_barang + "</td>"
                detail += "<td>" + isi.jumlah_masuk + "</td>"
                detail += "</tr>"
            ...
            $('#modalp .modal-body').html(
                '<embed src="<?php echo base_url(); ?>cetak/pdf-barang-masuk.php" width="100%" height="500" type="application/pdf">');
            $('#modalp').modal('show');
            ...
            $('#modalp .modal-body').html(
                '<embed src="<?php echo base_url(); ?>cetak/pdf-barang-masuk-det.php" width="100%" height="500" type="application/pdf">');
            $('#modalp').modal('show');
        }
    }
    ...
    $('#btnSubmit').on('click', function(e) {
        ...
        $.ajax('<?= base_url() ?>load/barang-masuk/simpanubah.php', {
            dataType: 'json',
            type: 'POST',
            data: datatosend,
            cache: false,
    ...
    function BarisBaru() {
        $('.barangs').empty()
        var dt_getbrg = getJSON('<?= base_url() ?>load/barang-masuk/getallbrg.php', {
            id_s: $('#id_supp').val()
        });
        ...
        Baris += "<td><input type='text' class='form-control eachjml' name='jml[]' onkeypress='return check_int(event)' ></td>";

        Baris += "<td><input type='text' class='form-control keterangann' name='keterangann[]'></td>";
    ...
    function check() {
        var kdmasuk = $('#kdbrgmsk').val()
        var terima = $('#trmbrg').val()
        var supp = $('#id_supp').val()
        var tgl = $('#tgl').val()
        var ket = $('#ket').val()
